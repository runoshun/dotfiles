...
async function handler(req: Request): Promise<Response> {
  ...
  try {
    ...
    const decodedBody = new TextDecoder().decode(await req.arrayBuffer());
    const decodedJson = JSON.parse(decodedBody);
    delete decodedJson["model"];
    log.debug(
      `/chat/completions: ${req.method} ${req.url} body: ${decodedBody}`,
    );
